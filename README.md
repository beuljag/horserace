...
- **GUI 레이스 연출** — 상자 GUI 5개 레인에서 약 10초간 경기가 진행된다
- **이동 확률 레이스** — 매 틱 말마다 움직일지 굴린다. 배당이 낮을수록 잘 움직이고 높을수록 자주 멈춘다
- **실시간 실제 승률** — 실제 치러진 경기 결과를 집계해 승률로 표시한다 (`/hr stats`)
...
| `/hr stats` | 실제 경기 통계 (말별 우승 횟수·비율) | - |
| `/hr stats reset` | 실제 승률 통계 초기화 | `horserace.admin` |
...
2. 말 클릭 → 금액 클릭 → **즉시 경기장 GUI 가 열리고 레이스 시작** (약 10초).
...
- 우승마를 미리 정하지 않는다. **매 틱마다 말 하나하나가 "움직일지"를 굴리고**, 먼저 결승선을 넘는 말이 이긴다.
  베팅한 말이 무엇인지는 결과에 영향이 없다. 숨은 보정도 없다.
- 한 틱에 움직일 확률 = **(1 / 배당) ^ move-chance-power**. 기본(지수 0.5) 기준
  1.1배 → 95%, 1.3배 → 88%, 1.5배 → 82%, 1.7배 → 77%, 2.0배 → 71%.
  움직이면 트랙의 `move-step` 비율(±30% 흔들림)만큼 전진한다.
- 승률은 이 규칙에서 저절로 나오는 값이라 공식이 없다. 플러그인이 켜질 때와 배당을 바꿀 때
  `estimate-samples` 판을 돌려 추정하고, GUI 와 `/hr horses` 에 그 값을 표시한다.
  기본값(지수 0.5, 이동 거리 0.16) 기준 약 46% · 24% · 15% · 9% · 6%.
- **실시간 실제 승률** — 서버에서 실제로 치러진 경기의 우승마를 `stats.yml` 에 집계한다.
  `stats-min-races`(기본 30) 이상 쌓이면 예상값 대신 실제 우승 비율을 승률로 표시하고, 경기가
  끝날 때마다 갱신된다. 배당이나 규칙을 바꾸면 통계는 자동 초기화된다. `/hr stats` 로 확인.
- `move-step` 을 키우면 운이 더 작용해 배당 높은 말도 가끔 이기고, 줄이면 배당 낮은 말이 거의 항상 이긴다.
  `move-chance-power` 를 키우면 배당 높은 말이 더 자주 멈춘다.
- 배당이 승률의 역수보다 낮으면 플레이어가 불리하다. 관리자가 `/hr horses` 를 치면
...
race:
  move-chance-power: 0.5       # 이동 확률 = (1/배당)^이 값. 클수록 배당 높은 말이 자주 멈춤
  move-step: 0.16              # 한 번 움직일 때 전진 거리 (트랙 비율). 클수록 운이 많이 작용
  estimate-samples: 5000       # 표시 승률 추정에 돌리는 판 수 (실제 경기가 쌓이기 전)
  stats-min-races: 30          # 이 수 이상 실제 경기가 쌓이면 실제 우승 비율을 승률로 표시
  tick-interval-ticks: 32      # 레이스 진행 주기
