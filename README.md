...
`/경마` 로 말을 고르고 금액을 누르면 **바로 출발**한다. 대기 시간이나 정해진 경기 시각은 없고,
끝나면 결과 화면에서 **같은 베팅 한 번 더 / 다른 말 고르기** 로 계속 이어서 할 수 있다.
말마다 관리자가 정한 **고정 배당**(예: 1.1배, 1.3배, 1.5배, 1.7배, 2배)이 있고,
**배당이 높은 말일수록 승률이 낮다.** 베팅 재화는 블랙잭과 같은 방식으로
**아이템(기본 금괴)** 또는 **Vault 이코노미(돈)** 중 고를 수 있다.

> **⚠ 상태: 실서버 미검증.** 빌드와 로직은 확인했지만 아직 실제 운영 서버에서 돌려보지
> 않았다. 테스트 서버에서 먼저 확인하고 쓰길 권한다. 버그 제보는 Issues 로.
...
- `hidden-win-penalty` 는 실제 판정에서 **베팅한 말의 가중치에만** (1 − 이 값)을 곱해 살짝 불리하게 한다.
  GUI 와 `/hr horses` 에 표시되는 승률에는 반영되지 않는다. 기본 5%.
...
race:
  ...
  hidden-win-penalty: 0.05     # 실제 승률 = 표시 승률 × (1 - 이 값). 표시엔 안 나옴
